...
byte ReflectanceSensor::updateLeftLineState()
{
  if (leftOverLine())
  {
    if (leftState == LOW || leftState == FALLING)
      return (leftState = RISING);
    else
  ...
  {
    if (leftState == LOW || leftState == FALLING)
      return (leftState = LOW);
    else
      return (leftState = FALLING);
  }
}
/**
...
byte ReflectanceSensor::updateRightLineState()
{
  if (rightOverLine())
  {
    if (rightState == LOW || rightState == FALLING)
      return (rightState = RISING);
    else
  ...
  {
    if (rightState == LOW || rightState == FALLING)
      return (rightState = LOW);
    else
      return (rightState = FALLING);
  }
}
/**
